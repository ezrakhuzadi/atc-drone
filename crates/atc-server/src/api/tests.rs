...
#[tokio::test]
async fn telemetry_cannot_spoof_owner_id() {
    ...
    let token = register_body["session_token"]
        .as_str()
        .expect("session token");
    let drone_id = register_body["drone_id"].as_str().unwrap().to_string();
    ...
    let stored = state.get_drone("DRONE_OWNER_SPOOF").expect("drone state");
    assert_eq!(stored.owner_id.as_deref(), Some("owner-1"));
}
