...
async fn test_register_and_send_telemetry() {
    ...
    let resp = client
        .post(format!("{}/v1/drones/register", base))
        .json(&body)
        .send()
        .await
    ...
    let found = drones
        .iter()
        .any(|d| d["drone_id"].as_str() == Some("TEST-TEL-001"));
...
async fn test_telemetry_updates_position() {
    ...
    let drone = drones
        .iter()
        .find(|d| d["drone_id"].as_str() == Some(&drone_id))
        .unwrap();
